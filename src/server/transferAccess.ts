...
export function getTransferAccess(walletId: string, now = new Date()): TransferAccess {
  const account = getDb().prepare(`SELECT users.username, users.role, wallets.send_blocked_until, wallets.transfer_cooldown_exempt
  ...
  const rollingEnd = account.role !== "ADMIN" && !cooldownExempt && limited.created_at
    ? Date.parse(limited.created_at) + 86_400_000 : 0;
  const manualEnd = cooldownExempt ? 0 : account.send_blocked_until ? Date.parse(account.send_blocked_until) : 0;
