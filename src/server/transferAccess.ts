...
export function getTransferAccess(walletId: string, now = new Date()): TransferAccess {
  const account = getDb().prepare(`SELECT users.role, wallets.send_blocked_until
    FROM wallets JOIN users ON users.id = wallets.user_id WHERE wallets.id = ?`)
    .get(walletId) as { role: string; send_blocked_until: string | null } | undefined;
  ...
  const rollingEnd = account.role !== "ADMIN" && limited.created_at
